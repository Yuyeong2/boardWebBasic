...
public class BoardCmtDAO {
    ...
    public static int updBoardCmt(BoardCmtEntity entity) {
        Connection con = null;
        PreparedStatement ps = null;
        String sql = "UPDATE t_board_cmt SET " +
                " ctnt = ? WHERE icmt = ? AND writer = ?";
        try {
            con = DbUtils.getCon();
            ps = con.prepareStatement(sql);
    ...
    public static int delBoardCmt(BoardCmtEntity entity) {
        Connection con = null;
        PreparedStatement ps = null;
        String sql = "DELETE FROM t_board_cmt WHERE writer = ? AND iboard = ? AND icmt = ?";

        try {
            con = DbUtils.getCon();
            ps = con.prepareStatement(sql);
            ps.setInt(1, entity.getWriter());
